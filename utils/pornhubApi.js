...
async function pornhubSearch(searched) {
  ...
  try {
    ...
    if (!contentList.results) {
      query = "taboo";
      contentList = await Pornhub.search(query, " ", { page: 3 });
    }
    console.log({ query, result: contentList.results[0] });
  } catch (e) {
  ...
  const page = await Pornhub.page(link, [
    "title",
    "description",
    "thumbnail_url",
  ]);
...
async function pornhubSearchGif(searched) {
  ...
  let query = searched;
  try {
    contentList = await Pornhub.search(query, ["title"], {
      search: "gifs",
      page: 3,
    });
